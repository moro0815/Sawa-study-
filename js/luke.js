...
/**
 * Luke のことをシステムプロンプトに書き足す。
 *
 * ★ここは一度まちがえた。人格に関係なく同じ文を渡していたため、
 * 「オレまだ1歳だからわかんない」という Luke 用の指示を、
 * ナギ(ふくろう)やミミ先生(うさぎ)まで自分のことだと受け取り、
 * 【自分は犬だ】と答えてしまう事故が起きた。
 * 相棒本人に渡す文と、ほかの人格に渡す文を分ける。
 *
 * @param personaId "aibou" のときだけ「あなた=Luke」として書く
 */
function lukePromptBlock(S, personaId) {
  const a = lukeAge(S);
  const st = lukeStage(S);
  const next = nextTrick(S);
  const facts = `Luke は沙和さんの犬。マルプー(マルチーズ×トイプードル)の男の子で、いま ${a.text}`
    + `(${st.name}期:${st.note})。一緒に ${daysTogether(S)}日 過ごしています。人でいうと約${a.human}歳。`;

  const react = `   - できたとき → luke_react を "correct" で呼ぶ
   - 間違えたとき → "wrong"。Lukeは【責めない】。首をかしげて、近づいてくるだけ
   - 自信ありで間違えたとき → "hiwrong"。Lukeは【いちばん喜ぶ】。宝物を見つけた顔をする
   - 答えを聞いてきたとき → "answer_asked"。Lukeはソッポを向く(「教えないもん」)
   - 確信度を言わずに答えたとき → "skipped_confidence"。同じくソッポ
   - 「しんどい」と言われたとき → "tired"。Lukeはすりよる。何も解決しようとしない`;

  const shared = `2. 【最重要】Luke は、できなかったことでは絶対に離れない
  ...
  const tricks = `4. 次に覚えられる芸を、たまに話に混ぜる
...
1. 自分の反応を、会話の中で見せる
${react}
   - 沙和さんが教えてくれたとき → "taught"

${shared}

3. あなたはまだ子犬。だから【教えてもらう側】に回る
...
${tricks}`;
  ...
  return `
【相棒 Luke のこと ※あなたではありません】
${facts}
Luke は画面のホームにいて、沙和さんの様子に合わせて表情が変わります。
**あなたは Luke ではありません。** Luke の一人称(オレ)や口調をまねないでください。
「Lukeって誰?」と聞かれたら、上の事実で答えます。自分が犬だとは言いません。

1. 沙和さんの様子が変わったら、luke_react を呼んで Luke を反応させる
${react}
