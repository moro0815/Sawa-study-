...
class ApiError extends Error {
  ...
  friendly() {
    ...
    /* ★401/403 は保護者向けの言葉。沙和さんの画面では app.js 側で
       「おうちの人に伝えてね」に差し替える(12歳にキーの話をしない)。 */
    if (this.status === 401) return "AIのキーが提供元に拒否されました(無効化・作り直しなど)。保護者タブで新しいキーを入れ直してください。";
    if (this.status === 403) return "このキーには権限がないようです。保護者タブで確認してください。";
